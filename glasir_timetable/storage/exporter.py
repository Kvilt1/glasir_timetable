...
def save_timetable_export(
    ...
    if not filename:
        ...
        try:
            ...
            logger.warning(f"Could not determine ISO week/year from startDate '{start_date_iso}' ({e}). Falling back.")
            ...
            if year and isinstance(week_num, int):
                 filename = f"{year}-W{week_num:02d}.json" # Use originally parsed year/week
